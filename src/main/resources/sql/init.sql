-- CRM 系统核心表结构
-- 执行前请先创建数据库: CREATE DATABASE crm_db DEFAULT CHARACTER SET utf8mb4;

USE crm_db;

-- =============================================
-- 1. 客户表
-- 设计要点：score/total_amount/order_count 为冗余字段，
--           由订单完成事件异步更新，避免每次实时计算。
-- =============================================
CREATE TABLE IF NOT EXISTS t_customer (
    ...
    credit_days  INT           NOT NULL DEFAULT 30   COMMENT '账期天数（合同约定）',
    overdue_count INT          NOT NULL DEFAULT 0    COMMENT '历史逾期次数（影响信用评分）',
...
-- =============================================
-- 2. 订单表
-- 设计要点：status 使用枚举字符串便于可读性；
--           delivery_date 作为预警触发的时间基准。
-- =============================================
...
-- =============================================
-- 4. 物流轨迹表
-- 设计要点：每次轨迹更新插入新记录（append-only），
--           便于展示完整物流时间线。
-- =============================================
CREATE TABLE IF NOT EXISTS t_logistics (
    id          BIGINT       NOT NULL AUTO_INCREMENT COMMENT '主键',
    order_id    BIGINT       NOT NULL COMMENT '关联订单ID',
    carrier     VARCHAR(32)           COMMENT '承运商: DHL/FedEx/UPS等',
...
-- =============================================
-- 5. 预警记录表
-- 设计要点：status 区分处理状态，notified_at 记录推送时间，
--           用于避免重复推送。
-- =============================================
CREATE TABLE IF NOT EXISTS t_alert (
    ...
    message      VARCHAR(512) NOT NULL COMMENT '预警消息内容',
    status       VARCHAR(16)  NOT NULL DEFAULT '未处理'
                 COMMENT '处理状态: 未处理/已处理',
    handler      VARCHAR(32)           COMMENT '处理人',
    handle_time  DATETIME              COMMENT '处理时间',
    notified_at  DATETIME              COMMENT '飞书推送时间（非空表示已推送）',
    create_time  DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    update_time  DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    INDEX idx_order_id (order_id),
    INDEX idx_status (status),
    -- 联合索引：优化"按客户查预警历史"场景（通过 order_id join）
